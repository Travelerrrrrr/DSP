 ...
 * @param[out] result_data packed RFFT 复数输出，长度至少为 FFT_LENGTH（CMSIS packed 格式：[DC, Nyquist, X1.re, X1.im, ...]）。
 * @param[in] window 窗函数类型。
 * @param[in] offset 转换为 float32 后叠加到每个采样点的偏置值；0.0f 表示不调整。
 * @note 该 packed 频域结果可直接传给 irfft_start 作为 rfft_complex 输入。参数非法或 RFFT 前处理失败时调用 RFFT_ERROR_HANDLER。
 */
 ...
 * @param[in,out] hrfft RFFT 服务句柄指针。
 * @param[out] result_data 缓冲区长度至少为 FFT_LENGTH（先存放 packed FFT，再原地压缩为单边幅频，最终结果在 result_data[0..RFFT_RESULT_LENGTH]）。
 * @param[in] window 窗函数类型。
 ...
 * @param[in,out] hrfft RFFT 服务句柄指针。
 * @param[out] result_data 缓冲区长度至少为 FFT_LENGTH（先存放 packed FFT，再原地压缩为单边相频，最终结果在 result_data[0..RFFT_RESULT_LENGTH]）。
 * @param[in] window 窗函数类型。
 ...
 * @param[in,out] hrfft RFFT 服务句柄指针；句柄内 scratch_buffer 在提取过程中被借用为相位暂存。
 * @param[out] result_data 缓冲区长度至少为 FFT_LENGTH + 2（先存放 packed FFT，再原地写出 [幅值 RFFT_RESULT_LENGTH+1 | 相位 RFFT_RESULT_LENGTH+1]）。
 * @param[in] window 窗函数类型。
 * @param[in] unit 幅值输出单位。
 * @param[in] p_unit 相位输出单位；0 表示弧度，非 0 表示角度。
 * @param[in] offset 转换为 float32 后叠加到每个采样点的偏置值；0.0f 表示不调整。

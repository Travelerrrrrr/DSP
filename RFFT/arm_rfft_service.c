...
#define RFFT_LOG_MIN FLT_MIN // 最小规格化正数，避开反规格化区（FZ=1 时反规格化会被刷为 0）
#define RFFT_LOG_MAX FLT_MAX
 ...
 * @param result_data 幅频结果数组，长度至少为 RFFT_HALF_LENGTH + 1。
 ...
 * @param hrfft RFFT 句柄。
 * @param result_data 输入为 packed RFFT 复数（长度 FFT_LENGTH），原地压缩为单边幅频结果（长度 RFFT_HALF_LENGTH+1）。
 * @note 普通正频率点按单边谱幅值缩放，DC 和 Nyquist 额外乘以 0.5 以避免被双边合并系数放大。
 ...
 * @param hrfft RFFT 句柄（未使用）。
 * @param result_data 输入为 packed RFFT 复数（长度 FFT_LENGTH），原地压缩为单边相频结果（长度 RFFT_HALF_LENGTH+1）。
 * @param unit 相位输出单位选择；0 表示弧度，非 0 表示角度。
 ...
 * @param hrfft RFFT 句柄。
 * @param result_data 输入为 packed RFFT 复数（长度 FFT_LENGTH），原地写出 [幅值0, 相位0, 幅值1, 相位1, ...]，总长度 FFT_LENGTH+2。
 * @param unit 相位输出单位选择；0 表示弧度，非 0 表示角度。
 * @note 普通正频率点复用原 packed bin 的 [real, imag] 位置写出幅值和相位；DC/Nyquist 先暂存实部，最后写回交错结果边界。
 */
static void rfft_make_single_sided_amplitude_phase(rfft_handle_t *hrfft, float32_t *result_data, uint8_t unit)
{
	...
	// 幅值和相位按 [幅值, 相位] 交错写回原 packed bin 位置
	for (i = 1U; i < RFFT_HALF_LENGTH; i++)
	{
		const float32_t real = src[0]; // 优化运行效率
		const float32_t imag = src[1];
	...
	// DC / Nyquist 写入交错结果边界，幅值不翻倍（scale*0.5 一次性应用）
	result_data[0] = fabsf(dc_real) * edge_scale;
 ...
 * @param hrfft RFFT 句柄。
 * @param adc_data_addr 输入采样缓冲区地址。
 * @param result_data packed RFFT 复数输出缓冲区，长度至少为 FFT_LENGTH。
 * @param window 窗函数类型。
 * @param offset 转换为 float32 后叠加到每个采样点的偏置值；0.0f 表示不调整。
...
/**
 * @brief 启动 RFFT 计算并输出 CMSIS packed 复数频域结果。
 * @param hrfft RFFT 句柄。
 * @param adc_data_addr 输入采样缓冲区地址。
 * @param result_data packed RFFT 复数输出缓冲区，长度至少为 FFT_LENGTH。
 * @param window 窗函数类型。
 * @param offset 转换为 float32 后叠加到每个采样点的偏置值；0.0f 表示不调整。
 * @note result_data 输出格式为 [DC, Nyquist, X1.real, X1.imag, ...]；参数非法或 RFFT 前处理失败时调用 RFFT_ERROR_HANDLER。
 */
 ...
 * @param result_data 工作/输出缓冲区，长度至少为 FFT_LENGTH；最终输出 result_data[0..RFFT_HALF_LENGTH]。
 * @param window 窗函数类型。
 * @param p_unit 相位输出单位选择；0 表示弧度，非 0 表示角度。
 * @param offset 转换为 float32 后叠加到每个采样点的偏置值；0.0f 表示不调整。
...
{
#if USE_ARM_FFT
	if ((hrfft == NULL) || (rfft_complex == NULL) || (irfft == NULL) || (AF == NULL) || (PF == NULL))
	{
		RFFT_ERROR_HANDLER();
		return;
	}
...
{
#if USE_ARM_FFT
	if ((hrfft == NULL) || (irfft == NULL) || (AF == NULL) || (PF == NULL))
	{
		RFFT_ERROR_HANDLER();
		return;
	}
	...
	// arm_rfft_fast_f32 inverse 模式内部 merge_rfft_f32 的访问模式不支持 in-place，
	// 输入/输出必须是不同 buffer
	arm_rfft_fast_f32(&hrfft->fft_instance, Complex_base, irfft, 1);

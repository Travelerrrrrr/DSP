...
FILTER_INFO filter_check(uint32_t addr, uint8_t Msize, uint16_t Length, float threshold, uint8_t BitWidth, float ADC_Ref)
{
    ...
    //     for (i = 0; i < Length; i++)
    //     {
    //         if ((float)(*(float *)(addr + i * 4)) < vFFTDATAThreshold)
    //         {
    //             fullpass = 0;
    //             break;
    //         }
    ...
    //     for (i = 1; i < Length - 1; i++)
    //     {
    //         if ((float)(*(float *)(addr + i * 4 - 4)) < vFFTDATAThreshold && (float)(*(float *)(addr + i * 4 + 4)) >= vFFTDATAThreshold)
    //         {
    //             filter_info.pRais = i;
    //             for (j = i + 2; j < Length - 1; j++)
    //             {
    //                 if ((float)(*(float *)(addr + j * 4 - 4)) >= vFFTDATAThreshold && (float)(*(float *)(addr + j * 4 + 4)) < vFFTDATAThreshold)
    //                 {
    //                     filter_info.pFall = j;
    //                     find = 1;
    //                     filter_info.type = FILTER_BANDPASS;
    ...
    //     for (i = 0; i < Length; i++)
    //     {
    //         if ((uint8_t)(*(uint8_t *)(addr + i * _addr_Inc)) < vFFTDATAThreshold)
    //         {
    //             fullpass = 0;
    //             break;
    //         }
    ...
    //     for (i = 1; i < Length - 1; i++)
    //     {
    //         if ((uint8_t)(*(uint8_t *)(addr + i * _addr_Inc - _addr_Inc)) < vFFTDATAThreshold && (uint8_t)(*(uint8_t *)(addr + i * _addr_Inc + _addr_Inc)) >= vFFTDATAThreshold)
    //         {
    //             filter_info.pRais = i;
    //             for (j = i + 2; j < Length - 1; j++)
    //             {
    //                 if ((uint8_t)(*(uint8_t *)(addr + j * _addr_Inc - _addr_Inc)) >= vFFTDATAThreshold && (uint8_t)(*(uint8_t *)(addr + j * _addr_Inc + _addr_Inc)) < vFFTDATAThreshold)
    //                 {
    //                     filter_info.pFall = j;
    //                     find = 1;
    //                     filter_info.type = FILTER_BANDPASS;
    ...
    //     for (i = 0; i < Length; i++)
    //     {
    //         if ((uint16_t)(*(uint16_t *)(addr + i * _addr_Inc)) < vFFTDATAThreshold)
    //         {
    //             fullpass = 0;
    //             break;
    //         }
    ...
    //     for (i = 1; i < Length - 1; i++)
    //     {
    //         if ((uint16_t)(*(uint16_t *)(addr + i * _addr_Inc - _addr_Inc)) < vFFTDATAThreshold && (uint16_t)(*(uint16_t *)(addr + i * _addr_Inc + _addr_Inc)) >= vFFTDATAThreshold)
    //         {
    //             filter_info.pRais = i;
    //             for (j = i + 2; j < Length - 1; j++)
    //             {
    //                 if ((uint16_t)(*(uint16_t *)(addr + j * _addr_Inc - _addr_Inc)) >= vFFTDATAThreshold && (uint16_t)(*(uint16_t *)(addr + j * _addr_Inc + _addr_Inc)) < vFFTDATAThreshold)
    //                 {
    //                     filter_info.pFall = j;
    //                     find = 1;
    //                     filter_info.type = FILTER_BANDPASS;
    ...
    //     for (i = 0; i < Length; i++)
    //     {
    //         if ((uint32_t)(*(uint32_t *)(addr + i * _addr_Inc)) < vFFTDATAThreshold)
    //         {
    //             fullpass = 0;
    //             break;
    //         }
    ...
    //     for (i = 1; i < Length - 1; i++)
    //     {
    //         if ((uint32_t)(*(uint32_t *)(addr + i * _addr_Inc - _addr_Inc)) < vFFTDATAThreshold && (uint32_t)(*(uint32_t *)(addr + i * _addr_Inc + _addr_Inc)) >= vFFTDATAThreshold)
    //         {
    //             filter_info.pRais = i;
    //             for (j = i + 2; j < Length - 1; j++)
    //             {
    //                 if ((uint32_t)(*(uint32_t *)(addr + j * _addr_Inc - _addr_Inc)) >= vFFTDATAThreshold && (uint32_t)(*(uint32_t *)(addr + j * _addr_Inc + _addr_Inc)) < vFFTDATAThreshold)
    //                 {
    //                     filter_info.pFall = j;
    //                     find = 1;
    //                     filter_info.type = FILTER_BANDPASS;
